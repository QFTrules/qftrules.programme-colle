\begin{tikzpicture} [
	...
	\draw (-2,0)--++(4,0);
	\draw (0,-1.75) node{liquide ($\rho_\ell$)};
%	\draw (0,1.2) node[above]{air $(P_\text{atm})$};
	% solide
	\draw[fill=white,shift={(-1.5,-10pt)}](0,0)rectangle(3,30pt);
	\draw (1,22pt) node[below,text width=1cm]{solide ($\rho_\text{s}$)};
	\draw[dashed,thin] (-1.5,0)--++(3,0);	
	\draw[shift={(-1.5,-10pt)},<->](10pt,0)--++(0,30pt)node[inner sep=0,midway,fill=white]{$h$};
