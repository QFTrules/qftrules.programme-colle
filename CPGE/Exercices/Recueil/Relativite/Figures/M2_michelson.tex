...
\def\ang{0}   % angle between x and x' axes
...
\def\axes{
  ...
  \draw[->,thick] (-\xmin,0) -- (X) node[below=0] {$y$};
  % \draw[->,thick,mydarkred] (90-\ang:-\xmin) -- (T')
    % node[right=5,above=-1] {$ct'$};
    \begin{scope}[xshift=3*\d cm]
      \draw[thick,myred] (90-\ang:-\xmin) -- (90-\ang:\ymaxp+0.2)
      node[right=5,above=-1,mydarkred] {$\mathcal{M}_2$};
    \end{scope}
...
\begin{tikzpicture}[scale=2]
  ...
  % \draw[mygreen,dashed,rounded corners=0.1]
    ...
    % \draw[photon]
    %  (C) -- (D);
    \draw[photon] (0,0) -- ++ (45:1.41*\d*3) coordinate (E);
    \draw[photon] (E) -- ++ (135:1.41*\d*3) coordinate (F);

    % coordinates
    % \draw[very thick,myred,rounded corners=0.1] (B) 
    % node[midway,above=0] {\contour{white}{$\Delta x'$}}
    % -- (A) node[pos=0.5,left=-2] {$c\Delta t_0$};
    \fill[mygreen] (F) circle(0.03) node[above right] {\contour{white}{$R_2$}};
    \node at (F) [left,mygreen] {\contour{white}{$2\gamma\ell$}};
    \fill[myblue] (A) circle(0.03) node[below=8, right=0] {\contour{white}{$E_0$}};
    \fill[myblue] (E) circle(0.03) node[below=1,right=0] {\contour{white}{$A_2$}};
    ...
    \draw[myblue,dashed,thick,rounded corners=0.1]
     (E)   -- ++ (\ang:-3.1*\d) node[left=-4] {\contour{white}{$\gamma \ell$}}; 
\draw[myblue,dashed,thick,rounded corners=0.1]
     (E)   -- ++(90-\ang:-3.1*\d) node[below] {\contour{white}{$\ell$}}; 
