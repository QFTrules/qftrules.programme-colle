...
\begin{tikzpicture}[scale=1.8]
  ...
  \end{pgfonlayer}
  \draw[->,thick,mygreen] (\xA,-\xmin) --++ (0,\xmin+\xmax);
  \draw[->,thick,myblue] (\xA+\Lz,-\xmin) --++ (0,\xmin+\xmax);
  
  % ROD
  \draw[rod] (\xA,0) --++ (L)
    node[pos=0.3,below=-1] {$L_0$};
  ...
  \fill[mygreen] (0,0) circle(0.03) node[below=1,left=-2.3] {\contour{myfieldred}{$E_1$}};
  ...
  \fill[myblue] (\Lz,0) circle(0.03) node[below=1,right=0] {\contour{myfieldred}{$E_2$}};
