...
plt.xticks([ i for i in n.linspace(xmin,xmax,nmbx)],
    [str('{}%.2f'%(i)).replace('.',',') for i in n.linspace(xmin,xmax,nmbx)]) # Ici vous pouver changer le nombre de decimales (.0f)
plt.xlim([xmin, xmax])
...
def si(x,A0=0):
    return A0 + 2*np.sin(0.5*2*np.pi*x)
def s(x,k=0.1,A0=0):
    return k*sp(x)*si(x,A0=A0)
...
# Save the figure
figsave('signaux_surmodul')   # Changer le nom de la figure
