...
\def\axes{
  ...
  % AXES
  \draw[<-,thick] node[above=10] {$ct$} (0,-\xmin) -- (T) ;
  \draw[<-,thick] node[right=10] {$x$} (-\xmin,0) -- (X) ;
...
\def\ang{15} % angle between x and x' axes
\begin{tikzpicture}[scale=-2.5]
  ...
  \end{pgfonlayer}
  \draw[<-,thick,mygreen] (0,0) --++ (90-\ang:\Lmax) node[below] {$\rm B$};
  % \draw[thick,mygreen] (0,0) --++ (270-\ang:\Lmax);
  \draw[<-,thick,myblue] (\Lz,0) --++ (90-\ang:\Lmax) node[below] {$\rm A$};
  ...
  \draw[myblue,dashed] (E1) -- (\el,0) node[above] {$x_1$};
