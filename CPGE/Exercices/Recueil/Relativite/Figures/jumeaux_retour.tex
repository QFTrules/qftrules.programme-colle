...
\def\axes{
  ...
  % AXES
  \draw[->,thick] (0,-\xmin) -- (T) node[above=-1] {$t$};
...
\begin{tikzpicture}[scale=2.0]
  ...
  \foreach \i [evaluate={\x=\i*\d;}] in {1,...,\Nlines}{
    ...
    \draw[world line t] (-\xmin, \x) -- (0.5*\xmax+\d, \x);
  }
  \foreach \i [evaluate={\x=\i*\d;}] in {1,...,3}{
    \message{  Running i/N=\i/\Nlines, x=\x...^^J}
    \draw[world line]   ( \x,-\xmin) -- ( \x,\ymax);
    % \draw[world line t] (-\xmin, \x) -- (0.5*\xmax, \x);
  }
  \draw[world line t] (-\xmin,{(\Nlines+1)*\d}) -- (0.5*\xmax +\d,{(\Nlines+1)*\d});
  
  % AXES
  \draw[->,thick] (0,-\xmin) -- (0,\ymax+0.2) node[above] {$t$};
  \draw[->,thick] (-\xmin,0) -- (0.5*\xmax+\d + 0.2,0) node[right] {$x$};
  
  % VECTORS
  % \draw[vector,shorten >=1] (A) -- (T2);
  \draw[vector,mydarkblue,shorten >=2] (C) --++ (-\D,\dt)  node[midway,above=1,right=1] {$\frac{T_2}{2}$};
  % \draw[vector,mydarkblue,shorten >=1] (C) -- (T2) node[midway,above=1,right=1] {$\frac{T_2}{2}$};
  \draw[<->,very thick,mydarkblue] (0,2*\dt) -- (0,2*\dt-\h) node[midway,left=-1] {\contour{white}{$t_R$}};
  % \draw[<->,very thick,mydarkblue] (0,2*\dt-\h) --++ (0,\h) node[midway,left=-1] {\contour{white}{$t_R$}};
  ...
  % \foreach \i [evaluate={\dy=(\i-1)*\ystep; \ya=\i*\ystep; \yb=2*\dt-\i*\ystep;}] in {3}{
    % \draw[mydarkblue,dashed,line width=0.6]
      % (0,\ya)++(90-\ang:-0*\xmin) --++ (90-\ang:{0*\xmin+\D/sin(\ang)});
    % \draw[mydarkblue,dashed,line width=0.6]
      % (0,\yb)++(\ang-90:-0*\xmin) --++ (\ang-90:{0*\xmin+\D/sin(\ang)});
    ...
    % \fill[mydarkred]  (C)++( \ang:{\dy*sin(\ang)/cos(2*\ang)}) circle(0.02);  1
  % }
  \fill[mydarkblue]  (0,2*\dt-\h) circle(0.02) node[below left =-1] {\contour{white}{R}};
  \draw[black,dashed,line width=0.6]
      (0,\dt)++(90-\ang:-0*\xmin) --++ (\D,0);
  % \fill[mydarkred] (A) circle(0.04) node[below left=-1] {0}; % observer A
  \draw[->,dashed,mydarkblue,line width=0.6] (C) -- ++ (-1.5*\D,1.5*\dt) node[above] {$t'$};
  \draw[->,dashed,mydarkblue,line width=0.6] (C) -- ++ (-\dt,\D) node[left] {$x'$};
  
  % \fill[mydarkblue] (B) circle(0.04) node[below right=-1] {0}; % observer B
  % observer B returns
  ...
  % TICKS
  \node[fill=white,inner sep=1,above=1,left=3] at (T1) {$\frac{T_1}{2}$};
  \node[fill=white,inner sep=1,above=2,left=3] at (T2) {$T_1$};
  \tick{T1}{0};
  \tick{T2}{0};

  \fill[mydarkgreen] (C) circle(0.04)
    node[below right] {\contour{white}{D}}; 
