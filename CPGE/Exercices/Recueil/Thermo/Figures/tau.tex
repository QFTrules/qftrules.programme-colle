...
\begin{tikzpicture}[scale=2.0]
  ...
  \draw[vector,mydarkgreen]
    (-0.10*\xmax,-0.12*\xmax) to[out=35,in=-100] (O)
    to[out=80,in=-80,looseness=1.5] (0.3*\xmax,1.05*\xmax)
    % node[mydarkgreen,above=-3] {\contour{white}{$x(t)=\int  v(t)\mathrm{d}t$}};
    node[mydarkgreen,above=-3] {\contour{white}{$\mathcal{O}$}};
  % \draw[vector,myred] (O) -- (0,0.88*\xmax)
    % node[mydarkred,below left=0] {\contour{white}{$x(t)=0$}};
  %\node[right=8,above,mydarkpurple] at (T) {$x(t)=0$};
  \coordinate (A) at (0.12*\xmax,0.36*\xmax);
  \coordinate (B) at ($(A) + (57:0.1)$);
  \draw[dashed] (A) -- (0.12*\xmax,0) node[below] {$\mathrm{d} x$};
  \draw[dashed] (B) -- ++ (0,-0.4*\xmax);
  \draw[dashed] (A) -- ++ (-0.12*\xmax,0);
  \draw[dashed] (B) -- ++ (-0.14*\xmax,0) node[left] {$c\mathrm{d} t$};
  \draw[|-|,mydarkorange,thick] (A) -- (B) node[midway,right] {$\mathrm{d} s = c \mathrm{d} \tau$};
