...
\def\axes{
  ...
  \draw[<-,thick] node[right=15] {$y$} (-\xmin,0) -- (X) ;
...
\begin{tikzpicture}[scale=-2.5]
  ...
  \end{pgfonlayer}
  \draw[<-,thick,mygreen] (0,-\xmin) --++ (90-\ang:\Lmax+\xmin) node[below] {\contour{white}{$\rm A$}};
  % \draw[thick,mygreen] (0,0) --++ (270-\ang:\Lmax);
  \draw[<-,thick,myblue] (\Lz,-\xmin) --++ (90-\ang:\Lmax+\xmin) node[below] {\contour{white}{$\rm C$}};
  % \draw[thick,myblue] (\Lz,0) --++ (270-\ang:\Lmax);
  
  % ROD
  \pgfmathsetmacro\el{\Lz/(1-tan(\ang))}
  \draw[rod] (\xA,0) --++ (L)
    node[pos=0.5,above] {\contour{white}{$a$}};
  \draw[white,opacity=0] ({\xA+\el*tan(\ang)},{+\el}) --++ (L) coordinate (E1);
  ...
  \fill[myblue] (E1) circle(0.03) node[above left] {\contour{white}{$E_1$}};
  \coordinate (E3) at ({\el*tan(\ang)},\el);
  % \fill[mygreen] (E3) circle(0.03) node[above] {\contour{white}{$E_3$}};
  \draw[myblue,dashed] (E1) -- (\el,0) node[above left] {$y_1$};
