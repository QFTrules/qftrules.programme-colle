...
\begin{tikzpicture}[rotate=90]
  ...
  \fill[ball color=red!80,fill opacity=0.5] (-\L/2,-\W/2) to[out=180,in=180] ++ (0,\W) to[out=0,in=0] cycle;
  ...
  % ROD
  \draw[ball color=red!80,fill opacity=0.5] (-\L/2,-\W/2) --++(\L,0) to[out=0,in=0] ++ (0,\W) --++ (-\L,0) to[out=0,in=0] cycle;
  \foreach \i [evaluate={\x=-\L/2+(\i-0.4)*\L/\N;}] in {1,...,\N}{
    \node[scale=0.6,minuscol] at (\x,0) {$+$};
  }
