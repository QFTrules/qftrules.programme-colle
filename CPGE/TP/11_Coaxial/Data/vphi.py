...
plt.ylabel(r'$v_\varphi$ ($10^8$ m$\cdot$s$^{-1}$)')
...
f     = n.array(data[:,0])
phi   = n.array(data[:,1])               # Nom premiere variable
N     = n.shape(f)[0]            # Taille de la liste

c = 1.5
L = 50
phi  = phi*n.pi/180
vphi = 2*n.pi*f*1000*L/phi*1e-8
y = c/vphi
x = 1/(2*n.pi*f*1000)**2
...
v_err   = n.zeros(N) + 0.01
x_err   = n.zeros(N) + 0.01      # Mettre t/100 si 1% d'incertitude par ex.
y_err   = n.zeros(N) + 0.01
...
ploterr(size, mark, couleur,    # NE PAS TOUCHER
        f,                      # Abscisse
        vphi,                 # Ordonnee (ici je retranche l'origine des ordonnees par commodite mais c est juste pour l'exemple)
        f_err,                  # Incertitude abscisse
        v_err,                  # Incertitude ordonnee
...
# Save the figure
figsave('vphi_vs_f')   # Changer le nom de la figure
