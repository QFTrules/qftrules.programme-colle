...
\begin{tikzpicture}
  ...
  % Axes
  \def\ang{40}
  % \draw[->,>=latex] (0,0) -- ++ (-90-\ang:0.8*\r) node[below] (x1) {$x$};
  ...
  \begin{scope}
    ...
    \def\largeur{0.8}
    \pgfmathsetmacro{\J}{1.3*\R}
    % \draw[charge+] (-\a,0) circle (9pt) node[black,scale=1.0] {$+q$};
    % \draw[charge+] (+\a,0) circle (9pt) node[black,scale=1.0] {$+q$};
    \fill[ball color=red!80,fill opacity=0.5,draw opacity=0] (-\J,-0.7) -- ++(2*\R,0) -- ++ (90-\ang:2*\largeur) -- ++ (-2*\R,0) -- cycle;
    ...
    \pgfmathsetmacro{\K}{\J-0.3}
    \foreach \x in {0.2,0.6,...,\K}{
      \foreach \y in {-0.5,-0.1,0.3,...,0.5}{
        % draw a cross symbol
        \draw[minuscol] (-\x+0.8*\y+0.3,\y-0.1) -- ++(0,0.2);
        \draw[] (-\x-0.1+0.8*\y+0.3,\y) -- ++(0.2,0);
        \draw[] (\x+0.8*\y+0.3,\y-0.1) -- ++(0,0.2);
        \draw[] (\x-0.1+0.8*\y+0.3,\y) -- ++(0.2,0);
        }
      }
  \end{scope}
