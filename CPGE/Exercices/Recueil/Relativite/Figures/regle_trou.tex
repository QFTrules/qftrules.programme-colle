...
% beta = 0.85
\def\ang{40.4}   % angle between x and x' axes
...
\def\axes{
  ...
  \draw[->,thick,mydarkred] (90-\ang:-3) -- (90-\ang:3)
    node[right=5,above=-1] {$ct'$};
  \draw[->,thick,mydarkred] (\ang:-3) -- (\ang:3)
...
\begin{tikzpicture}[scale=1.8]
  ...
  \begin{pgfonlayer}{back} % draw on back
    \fill[mylightblue2] (270-\ang:\Lmax) ++ (0:-\Lz/2) --++ (90-\ang:2*\Lmax) -- ++ (0:\Lz) --++ (270-\ang:2*\Lmax) -- cycle;
  \end{pgfonlayer}
  \draw[->,thick,mygreen] (-\Lz/2,0) --++ (90-\ang:\Lmax*1.1);
  \draw[thick,mygreen] (-\Lz/2,0) --++ (270-\ang:\Lmax*1.1);
  \draw[->,thick,mygreen] (\Lz/2,0) --++ (90-\ang:\Lmax*1.1);
  \draw[thick,mygreen] (\Lz/2,0) --++ (270-\ang:\Lmax*1.1);

  \draw[->,thick,myblue] (-\Lz,0) --++ (90:\Lmax);
  \draw[thick,myblue] (-\Lz,0) --++ (270:\Lmax);
  \draw[->,thick,myblue] (\Lz,0) --++ (90:\Lmax);
  \draw[thick,myblue] (\Lz,0) --++ (270:\Lmax);
  
  % ROD
  \pgfmathsetmacro\el{0.4*\Lz/(1-tan(\ang))}
  \draw[rod,draw=mygreen,double=mygreen] (-\Lz/2,-0.02) --++ (L)    node[pos=0.75,below,mygreen] {\contour{white}{$L_0’$}};
  \draw[rod,draw=myblue,double=myblue] (-\Lz,0.02) --++ (2*\Lz,0)    node[pos=0.25,above,myblue] {\contour{white}{$2L_0’$}};

  \begin{scope}[yshift=1]
    \draw[rod,draw=myblue,double=myblue] (\ang:-1.3*\Lz) --++ (\ang:2.6*\Lz)  node[pos=0.8,below right,myblue] {\contour{white}{$L_0’$}};
  \end{scope}
  \begin{scope}[yshift=-1]
    \draw[rod,draw=mygreen,double=mygreen] (\ang:-2.4*\Lz) --++ (\ang:4.8*\Lz)  node[pos=0.8,below right,mygreen] {\contour{white}{$2L_0’$}};
  \end{scope}

  \coordinate (E) at (\Lz,{\Lz*cos(\ang)+\Lz});
  \node at (E) {$\bullet$}; 
  \node[left] at (E) {\contour{white}{$E_1$}};
  \coordinate (F) at (-\Lz,{-\Lz*cos(\ang)-\Lz});
  \node at (F) {$\bullet$};
  \node[right] at (F) {\contour{white}{$E_2$}};
