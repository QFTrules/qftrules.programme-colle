...
\def\axes{
  ...
  \draw[->,thick] (-\xmin,0) -- (X) node[right] {$y$};
...
\begin{tikzpicture}[scale=2.5]
  ...
  \coordinate (C) at (90+45:{4*\d*sqrt(2)});
  ...
  \draw[mydarkred,ultra thick] (0,-\xmax)  node[below] {$\mc{L}(B)$} -- (0,\ymax);
  \draw[mydarkred,ultra thick] (-4*\d,-\xmax) node[below] {$\mc{L}(A)$} -- (-4*\d,\ymax) ;
  % TRIANGLE
