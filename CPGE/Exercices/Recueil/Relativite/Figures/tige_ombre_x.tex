%r Author: Izaak Neutelings (October 2021)
% Inspiration
...
\def\axes{
  \pgfmathsetmacro\d{\xmin/(\Nlines+0.4)} % grid size
  \pgfmathsetmacro\D{\d/cos(\ang)/sqrt(1-tan(\ang)^2)} % grid size, boosted
  \pgfmathsetmacro\ymax{\xmin+\DNy*\d} % maximum of y = ct axis
  \pgfmathsetmacro\xmaxp{(\xmin/\d+\DNxp)*\D} % maximum of x' axis
  \pgfmathsetmacro\ymaxp{(\xmin/\d+\DNyp)*\D} % maximum of y' = ct' axis
  ...
  \draw[->,thick,mydarkred] (90-\ang:-\xmax) -- (T')
    node[right=5,above=-1] {$ct'$};
  \draw[->,thick,mydarkred] (\ang:-\xmax) -- (X')
...
%      (C) -- (D);
  ...
  \fill[myblue] (A) circle(0.03) node[below right] {\contour{white}{$E_1$}};
  \fill[myblue] (C) circle(0.03) node[above left] {\contour{white}{$E_2$}};
  ...
  % world line of corner B
  \draw[mydarkgreen,ultra thick] (90-\ang:-0.5)  node[below] {\contour{white}{$\mc{L}(B)$}} -- ++ (90-\ang:3);
  % world line of corner A
  \draw[mydarkgreen,ultra thick] ($(-4*\d,0) + (90-\ang:-0.5)$) node[below] {\contour{white}{$\mc{L}(A)$}} -- ++ (90-\ang:3);

  \draw[ultra thick, white, photon] (0,-\xmax) -- ++ (0,7*\d);
  \draw[ultra thick, white, photon] (-2.2*\d,-\xmax) -- ++ (0,7*\d);
  \draw[photon] (0,-\xmax) -- ++ (0,7*\d);
  \draw[photon] (-2.2*\d,-\xmax) -- ++ (0,7*\d);
  % longueur ombre b
  \draw[myorange, ultra thick] (0,0) -- (-2.2*\d,0) node[above,midway] {\contour{white}{$\ell$}};
