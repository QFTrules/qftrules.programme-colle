\input{/home/eb/Dropbox/texmf/tex/latex/Preambles/tikz.tex}
%3D
\usepackage{tikz-3dplot}

%Tikz Library
\usetikzlibrary{angles, quotes, intersections}

%Styles
\tikzset{axis/.style={thick,-latex}}
\tikzset{vec/.style={thick,blue}}
\tikzset{univec/.style={thick,red,-latex}}

%Notation
\usepackage{physics}
\usepackage{bm}

\begin{document}

	\tdplotsetmaincoords{70}{110}
	%
	\pgfmathsetmacro{\thetavec}{48.17}
	\pgfmathsetmacro{\phivec}{63.5}
	%

	\begin{tikzpicture}[tdplot_main_coords,scale=0.75]
		%Axis
		\draw[axis] (0,0,0) -- (6,0,0) node [pos=1.1] {$x$};
		\draw[axis] (0,0,0) -- (0,6,0) node [pos=1.05] {$y$};
		\draw[axis] (0,0,0) -- (0,0,5.5)  node [pos=1.05] {$z$};

		%Help Lines
		\draw[dashed] (2,4,4) -- (2,4,0);
		\draw[dashed] (2,0,4) -- (2,0,0);
		\draw[dashed] (2,4,4) -- (0,4,4);
		\draw[dashed] (2,0,4) -- (0,0,4);
		\draw[dashed] (2,4,4) -- (2,0,4);
		\draw[dashed] (0,4,4) -- (0,4,0);
		% \draw[dashed] (2,4,0) -- (0,0,0);
		\draw[dashed] (2,0,0) -- (2,4,0) node [pos=-0.1] {$x$};
		\draw[dashed] (0,4,0) -- (2,4,0) node [pos=-0.7, left] {$y$};
		\draw[dashed] (0,0,4) -- (0,4,4) node [pos=-0.1] {$z$};
		% \draw[dashed, tdplot_main_coords] (4.47,0,0) arc (0:90:4.47);

		%Unit Vectors
				% \draw[vec] (0,0,0) -- (2,4,4);
		\tdplotsetcoord{P'}{1.5}{90}{0}
	    	% \draw[univec] (2,4,0) -- ($(P')+(2,4,0)$) node [pos=1.3] {$\er$};
    	\tdplotsetcoord{P''}{1.5}{90}{90}
    	% \draw[univec] (2,4,0) -- ($(P'') + (2,4,0)$) node [pos=1.3] {$\et$};

			% \tdplotsetcoord{P'}{1}{90}{\phivec}
					\draw[univec] (2,4,4) -- ++ (P') node [pos=1.6] {$\ex$};
				% \tdplotsetcoord{P''}{1}{90}{90+\phivec}
				\draw[univec] (2,4,4) -- ++ (P'') node [pos=1.3] {$\ey$};
				\draw[univec] (2,4,4) -- ++ (0,0,1.5) node [pos=1.3] {$\ez$};

				\bull[above left]{(2,4,4)}{M};
				\bull[above left]{(0,0,0)}{O};



    	%Vectors
		% \tdplotsetcoord{P}{6}{\thetavec}{\phivec}
		% \draw[vec] (0,0,0) -- (P);
		% \draw[thick] (0,0,0) -- (2,4,0) node [pos=0.6, above] {$r$};

		%Point
		\node[fill=black, circle, inner sep=0.8pt] at (2,4,4) {};

		%Angles
		% \tdplotdrawarc{(0,0,0)}{0.7}{0}{\phivec}{below}{$\theta$}

	\end{tikzpicture}

\end{document}
