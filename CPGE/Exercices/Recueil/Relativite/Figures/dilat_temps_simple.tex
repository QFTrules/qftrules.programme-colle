...
\begin{tikzpicture}[scale=1.8]
  ...
  \draw[very thick,myorange,rounded corners=0.1,<->]
     (A) -- (C) node[midway,right=-2] {\contour{myfieldred}{$cT'$}};
  \draw[very thick,myorange,rounded corners=0.1,<->] (B) 
        % node[midway,above=0] {\contour{white}{$\Delta x'$}}
        -- (A) node[pos=0.5,left=-2] {$cT_0$};
  \fill[mygreen] (A) circle(0.03) node[above left] {\contour{white}{$E_1$}};
  \fill[myblue] (B) circle(0.03) node[below=1,left=0] {$E_2$};
