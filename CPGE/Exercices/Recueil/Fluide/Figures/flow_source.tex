...
\begin{tikzpicture}
  ...
  % draw exact field lines for a source flow : radial lines from origin
  % use foreach theta from 0 to 360 degrees every 30 degrees
  \foreach \theta in {0,30,...,330}{
    \draw[myblue,thick,postaction={decorate},decoration={markings,
      mark=at position {0.3} with {\arrow{latex}},mark=at position {0.6} with {\arrow{latex}},mark=at position 0.9 with {\arrow{latex}}}]
      (0,0) -- (\theta:\W/2);
  }
