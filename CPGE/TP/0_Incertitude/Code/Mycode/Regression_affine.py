...
def resbis(a,x,y,dx,dy):                        # résidu avec incertitude
    return res(a,x,y)/np.sqrt(dy**2+a[0]**2*dx**2)
...
print("Régression affine Y = aX + b")
print("a = {:.1e} ± {:.0e}".format(a,Da))
print("b = {:.1e} ± {:.0e}".format(b,Db))
print("khi2 réduit = {:.1e}".format(khi2red))