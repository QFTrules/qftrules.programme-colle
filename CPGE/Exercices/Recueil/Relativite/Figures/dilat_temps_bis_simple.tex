...
\begin{tikzpicture}[scale=1.8]
  ...
  % SETTINGS
  \pgfmathsetmacro\dt{4*\d} % time difference in S
  ...
  \coordinate (A') at ($(A)+(0,0.17)$);
  \coordinate (B') at ($(B)+(0,0.35)$);
  \coordinate (C) at ($(B)+(\ang:\dxp)$);
  \coordinate (D) at ($(C)+(0:-0.9*\dxp)$);
  ...
  \draw[mygreen,dashed,rounded corners=0.1]
    ...
    \draw[myblue,dashed,rounded corners=0.1]
     (C)   -- (B'); 
  \draw[very thick,myorange,rounded corners=0.1,<->]
     (A) -- (C) node[midway,right=-2] {\contour{white}{$cT'_0$}};
  \draw[very thick,myorange,rounded corners=0.1,<->] (D) 
        % node[midway,above=0] {\contour{white}{$\Delta x'$}}
        -- (A) node[pos=0.5,left=-2] {$cT$};
  % \fill[myblue] (C) circle(0.03) node[below=1,right=0] {$E_2'$};
  \fill[mygreen] (A) circle(0.03) node[below right=0] {\contour{white}{$E_1'$}};
