...
\begin{tikzpicture}[scale=1.8]
  ...
  \coordinate (C) at (\dt,1.333*\dt);
  \coordinate (D) at ($(C) + (-0.5*\dt,-1.333*0.5*\dt)$);
  
  \draw[->,thick] (\dt,0) -- ++(0,\ymax) node[above] {$y'$};
  \draw[->,thick] (0,1.333*\dt) -- ++(\xmax,0) node[right] {$x'$};
  ...
  % \draw[mygreen,dashed,rounded corners=0.1]
    % (A) -- (D) ;
    \draw[black,rounded corners=0.1]
     (0,0)   -- ++(\xmax,0); 
% \draw[myblue,dashed,rounded corners=0.1]
    %  (C)   -- ++(0,-1.2) node[below=0] {\contour{white}{$x_2$}}; 
  \draw[photon]
     (C) -- (D);
  \draw[black,dashed,rounded corners=0.1]
     (D)   -- (0,0);
  \draw[->] (0.5*\dt,1.333*\dt) arc(180:180+50:0.5*\dt) node[midway,left] {\contour{white}{$\theta_0'$}};
  % \draw[very thick,myred,rounded corners=0.1] (B) 
        ...
        % \fill[myblue] (C) circle(0.03) node[below=1,right=0] {\contour{white}{$E$}};
        \draw[mydarkblue,very thick,->] (0,0) -- ++(0.8,0) node[midway,above] {\contour{white}{$u$}};
        \fill[mygreen] (A) circle(0.03) node[] { 
          \includegraphics[width=0.6cm]{earth.png}
        };
        \fill[myblue] (C) circle(0.03) node {
    \includegraphics[width=0.4cm]{star.png}
  };
