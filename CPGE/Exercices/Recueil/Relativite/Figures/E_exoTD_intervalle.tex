...
\begin{tikzpicture}[scale=2.0]
  \message{Worldlines^^J}
    ...
    node[mydarkorange,left=1,above=-2] {\contour{white}{photon}};
     \draw[photon,shorten >=1] (O) -- (-45:1.5*\xmax)
    node[mydarkorange,left=1,below=-2] {\contour{white}{photon}};
    ...
    \node at (E) [above left,mydarkpurple] {\contour{white}{$E$}};
    \draw[dashed] (E) --  (\xe,0) node [below] {\contour{white}{$4$}};
    \draw[dashed] (E) -- (0,{\ye}) node [left] {\contour{white}{$2$}};

    \draw plot[domain={sqrt(12)*\d}:6*\d,samples=200] (\x,{sqrt(\x^2-12*\d^2)});
  \draw plot[domain={sqrt(12)*\d}:6*\d,samples=200] (\x,{-sqrt(\x^2-12*\d^2)});
\end{tikzpicture}
