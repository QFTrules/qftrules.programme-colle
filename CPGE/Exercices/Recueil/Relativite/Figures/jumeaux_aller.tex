...
\begin{tikzpicture}[scale=2.0]
  ...
  \draw[->,thick] (-\xmin,0) -- (\xmax+0.2,0) node[right] {$x$};
  
  % VECTORS
  % \draw[vector,shorten >=1] (A) -- (T2);
  \draw[vector,mydarkred,shorten >=2] (A) -- (C) node[midway,above=1,right=1] {$\frac{T_2}{2}$};
  % \draw[vector,mydarkblue,shorten >=1] (C) -- (T2) node[midway,above=1,right=1] {$\frac{T_2}{2}$};
  \draw[<->,very thick,mydarkred] (0,0) -- (0,\h) node[midway,left=-1] {\contour{white}{$t_A$}};
  ...
  \foreach \i [evaluate={\dy=(\i-1)*\ystep; \ya=\i*\ystep; \yb=2*\dt-\i*\ystep;}] in {3}{
    ...
    \fill[mydarkred]  (0,\ya) circle(0.02) node[left] {\contour{white}{A}};
  ...
  \draw[->,dashed,mydarkred,line width=0.6] (C) -- ++ (\D,\dt) node[above] {$t'$};
  \draw[->,dashed,mydarkred,line width=0.6] (C) -- ++ (\dt,\D) node[right] {$x'$};
