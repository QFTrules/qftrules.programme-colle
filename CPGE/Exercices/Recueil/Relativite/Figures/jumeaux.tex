...
\def\axes{
  ...
  % AXES
  \draw[->,thick] (0,-\xmin) -- (T) node[above=-1] {$ct$};
...
\begin{tikzpicture}[scale=2.0]
  ...
  % AXES
  \draw[->,thick] (0,-\xmin) -- (0,\ymax+0.2) node[above] {$ct$};
  \draw[->,thick] (-\xmin,0) -- (\xmax+0.2,0) node[right] {$x$};
  
  % VECTORS
  \draw[vector,shorten >=1] (A) -- (T2);
  \draw[vector,mygreen,shorten >=2] (A) -- (C) node[midway,above=1,right=1] {$\frac{T_2}{2}$};
  \draw[vector,mygreen,shorten >=1] (C) -- (T2) node[midway,above=1,right=1] {$\frac{T_2}{2}$};
  ...
  \fill[mydarkgreen] (C) circle(0.04)
    node[right=4] {\contour{white}{D}}; % observer B returns
