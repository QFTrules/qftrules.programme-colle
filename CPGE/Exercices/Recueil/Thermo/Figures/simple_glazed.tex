...
% Glass panes
\draw[fill=blue!20] (0,0) rectangle (3*\length,5) node[midway] {Verre};
% \draw[fill=blue!20] (2*\length,0) rectangle (3*\length,5) node[midway] {Verre};

% Air gap
% \draw[fill=blue!20] (\length,0) rectangle (2*\length,5) node[midway] {Verre};

% Width of glass and air gap
\draw[<->,shorten <=3pt, shorten >=3pt] (0,-0.5) -- (3*\length,-0.5) node[midway, below] {$\ell=3e$};
