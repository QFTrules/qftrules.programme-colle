...
\titreEntrainementFacultatif	{Un système d'équations}
\hauteurLargeurCadreReponse		{1cm}{4.4cm}
...
L'étude d'un problème de mécanique fait intervenir une force d'intensité inconnue $F$ et un angle inconnu $\alpha\in[0,\frac{\pi}{2}]$. La seconde loi de Newton projetée sur deux axes aboutit au système d'équations suivant :
	\[
	\systeme{T+F\sin\alpha=mR\omega^2,F\cos\alpha=mg}
	\]
...
\begin{corrige}
	...
	On a $\systeme{F\sin\alpha=mR\omega^2-T,F\cos\alpha=mg}$ soit $F^2(\sin^2\alpha+\cos^2\alpha)=F^2=\left(mR\omega^2-T\right)^2+(mg)^2$. Finalement, l'intensité d'une force étant positive, on trouve $F=\sqrt{\left(mR\omega^2-T\right)^2+(mg)^2}$.
\end{corrige}
...
\begin{corrige}
	Quand on écrit le système sous la forme $\systeme{F\sin\alpha=mR\omega^2-T,F\cos\alpha=mg}$, on s'aperçoit qu'il suffit de faire le rapport des deux équations pour éliminer $F$. On obtient 
	\[
...
\begin{center}
\subimport{_images/}{fiche_MCA02_figure_projection_1_JRL_v3.tex}
\end{center}
...
La composante suivant $\vv{e_x}$ correspond au produit scalaire $\vv{a}\cdot \vv{e_x}=a\times 1\times \cos(\alpha)$. De même la composante suivant $\vv{e_y}$ est le produit scalaire  $\vv{a}\cdot \vv{e_y}=a\times 1\times \cos(\pi/2-\alpha)=a\sin(\alpha)$. On peut retrouver ces résultats géométriquement (\emph{cf}. ci-contre).
...
%%                                                           %
\begin{center}
\subimport{_images/}{fiche_MCA02_figure_projection_2_JRL_v4.tex}
\end{center}
%% --------------------------------------------------------- %
%                               \finalisationDuPartageDePage %					
%% --------------------------------------------------------- %
...
\begin{corrige}
Le poids a pour composante suivant $\vv{e_x}$, $P_x=\vv{P}\cdot \vv{e_x}=P\cos(\alpha+\pi/2)=-P\sin(\alpha)$. De même sa composante suivant $\vv{e_y}$ s'écrit $P_y=\vv{P}\cdot \vv{e_y}=P\cos(\alpha+\pi)=-P\cos(\alpha)$. Ainsi, le poids s'écrit 
\[
...
\begin{corrige}
$\vv{N}$ est colinéaire au vecteur unitaire $\vv{e_y}$ et de même sens ; on a donc $\vv{N} = N \vv{e_y}$.
\end{corrige}
% ************************************** %
...
\hauteurLargeurCadreReponse		{7mm}{4.75cm}
\dureeResolutionFacultative		{2} % 1, 2, 3 ou 4
\typeEntrainement				{Newton} % Newton ou QCM ou AN ou vide (exercice "normal")
\nombreColonnesQuestions		{2} % vide, 1, 2, 3, etc.
\avecPlusieursQuestions			{Y} % Y ou N
...
\begin{corrige}
Le poids a pour composante suivant $\vv{e_r}$, $P_r=\vv{P} \cdot \vv{e_r}=P\cos(\theta)$. De même sa composante suivant $\vv{e_\theta}$ s'écrit $P_\theta=\vv{P}\cdot \vv{e_\theta}=P\cos(\alpha+\pi/2)=-P\sin(\theta)$. Ainsi, le poids s'écrit 
\[
...
\begin{corrige}
$\vv{T}$ est colinéaire au vecteur unitaire $\vv{e_r}$ et de sens opposé ; on a donc $\vv{T} =-T \vv{e_r}$.
\end{corrige}
% ************************************** %
...
\begin{corrige}
La tension du fil $\vv{T}$ a pour composante suivant $\vv{e_x}$, $T_x=\vv{T}\cdot \vv{e_x}=T\cos(\pi-\theta)=-T\cos(\theta)$. De même, sa composante suivant $\vv{e_y}$ vaut $T_y=\vv{T}\cdot \vv{e_y}=T\cos(\pi/2+\theta)=-T\sin(\theta)$. Finalement, on trouve $\vv{T}=-T\cos(\theta)\vv{e_x}-T\sin(\theta)\vv{e_y}$.
\end{corrige}
% ************************************** %
...
% •••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••• %
\sectionFicheEntrainement{De l'accélération à la position (et \emph{vice versa})}
% •••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••• %
...
\hauteurLargeurCadreReponse		{1.5cm}{3.5cm}
\dureeResolutionFacultative		{1} % 1, 2, 3 ou 4
...
\hauteurLargeurCadreReponse		{2cm}{3cm}
\dureeResolutionFacultative		{1} % 1, 2, 3 ou 4
...
\begin{corrige}
  D'après le PFD, $mg\vv{e_z}=m\vv{a}$ d'où $\vv{a}=g \vv{e_z}$.
\end{corrige}
...
\begin{corrige}
  ...
  \[
	...
	\right.
  \]
  Les conditions initiales imposent $C_1=v_0$, $C_2=0$ et $C_3=0$. Finalement $\vv{v}=v_0 \vv{e_x} + g t \vv{e_z} $.
\end{corrige}
...
\begin{corrige}
Le vecteur vitesse s'écrit $\vv{v}=\dot{x}\vv{e_x}+\dot{y}\vv{e_y}+\dot{z}\vv{e_z}$. Par identification avec l'expression obtenue précédemment, on a 
 \[
 \left\{
 \begin{array}{ccc}
 	\dot{x}&=&v_0\\
...
\right.
 \]
Les conditions initiales imposent $C_4=x_0$, $C_5=y_0$ et $C_6=0$. Finalement 
$$
...
% ********************************************************************* %
% ================ Métadonnées sur l'entraînement ===================== %
\titreEntrainementFacultatif	{Fondamental}
...
\begin{corrige}
  $\vv{e_r}\cdot\vv{e_x}=\cos(\theta)$ et  $\vv{e_r}\cdot\vv{e_y}=\cos(\pi/2-\theta)=\sin(\theta)$ d'où $\vv{e_r}=\cos(\theta) \vv{e_x} + \sin (\theta) \vv{e_y}$.
\end{corrige}
% ************************************** %
...
\begin{corrige}
 $\vv{e_\theta}\cdot\vv{e_x}=\cos(\pi/2+\theta)=-\sin(\theta)$ et  $\vv{e_\theta}\cdot\vv{e_y}=\cos(\theta)$ d'où $\vv{e_\theta}=- \sin(\theta) \vv{e_x} + \cos (\theta) \vv{e_y}$.
\end{corrige}
% ************************************** %
...
Exprimer dans la base $(\vv{e_x}, \vv{e_y})$ les vecteurs : 
...
\begin{corrige}
 Il suffit de dériver le vecteur $\vv{e_r}=\cos(\theta) \vv{e_x} + \sin (\theta) \vv{e_y}$ sachant que $\vv{e_x}$ et $\vv{e_y}$ sont des constantes (vectorielles). On a donc $\diff{\vv{e_r}}{t}=\diff{\cos(\theta)}{t} \vv{e_x} + \diff{\sin (\theta)}{t} \vv{e_y}$. Ici $\theta$ dépend du temps, par conséquent $\diff{\cos(\theta)}{t}=\diff{\theta}{t}\times \diff{\cos(\theta)}{\theta}=-\dot{\theta}\sin(\theta)$. de même $\diff{\sin(\theta)}{t}=\dot{\theta}\cos(\theta)$. Finalement, 
 \[
...
% ********************************************************************* %
% ================ Métadonnées sur l'entraînement ===================== %
\titreEntrainementFacultatif	{Deux dérivées à connaître}
\hauteurLargeurCadreReponse		{7mm}{4.5cm}
\dureeResolutionFacultative		{2} % 1, 2, 3 ou 4
\typeEntrainement				{Newton} % Newton ou QCM ou AN ou vide (exercice "normal")
\nombreColonnesQuestions		{2} % 1, 2, 3, etc.
...
Exprimer dans la base $(\vv{e_r}, \vv{e_\theta})$ les vecteurs : 
...
Exprimer en coordonnées polaires :

% ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ %
%               Question                 %
% ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ %
\begin{enonce}
	 le vecteur vitesse $\vv{v}$ 		
\end{enonce}

\reponse{$\dot{r} \vv{e_r} + r\dot{\theta} \vv{e_\theta}$}

\begin{corrige}
Il suffit de dériver le vecteur position en utilisant les résultats des exercices précédents : $\vv{v}=\diff{\vv{\ptO\ptM}}{t}=\diff{r}{t}\vv{e_r}+r\diff{\vv{e_r}}{t}=\dot{r} \vv{e_r} + r\dot{\theta} \vv{e_\theta}$.
\end{corrige}
...
\begin{corrige}
	...
	\]
	De plus, compte tenu de l'équilibre des forces 
	\[
		\vv{T}\cdot \vv{F}=
		(-\vv{F}-\vv{P})\cdot \vv{F}=
...
%                                                           %
%                                                           %
Un objet qui pèse \np[N]{800} est suspendu en équilibre à l'aide de deux cordes symétriques qui font un angle $\theta=\np[\degres]{20}$ avec la direction horizontale. 
...
\begin{corrige}
	$\vv{R}=\vv{T}+\vv{T'}+\vv{F}$. Sa composante horizontale vaut 
	\[
...
\begin{corrige}
	...
	\[
		...
		\end{array}	\right.
	\]
	Sachant que $F=\np[N]{800}$ et $\theta=20\degres$, on obtient $T=\np[kN]{1.17}$.
\end{corrige}
...
Un corps de masse $m=\np[kg]{2}$ tombe verticalement avec une accélération de $a= \np[m.s^{-2}]{9}$. Lors de sa chute il subit la force de pesanteur ainsi qu'une force de frottement due à l'air.

On prendra $g=\np[m.s^{-2}]{9,8}$ pour l'intensité du champ de pesanteur.
...
\begin{enonce}
Quelle est l'intensité de la force de frottement ?	 
\end{enonce}
...
Un homme de masse $m=\np[kg]{80}$ est dans un ascenseur. Cet ascenseur monte avec une accélération $a = \np[m.s^{-2}]{1}$. On note $\vv{F}$ la force exercée par l'homme sur le plancher de l'ascenseur.
...
\begin{enonce}
	 Quelle est l'intensité de $\vv{F}$ ?
\end{enonce}
...
Le plan est incliné d'un angle $\alpha$, comme figuré ci-contre.
% --------------------------------------------------------- %
%                                                           %
% -------------------- Partie à droite -------------------- %
%                                                           %
...
puis on maintient le bloc B$_{1}$ sur la table alors que l'autre est suspendu dans l'air. On libère le bloc B$_{1}$ qui glisse alors sur la table. On note $T_1$ et $T_2$ les tensions exercées par le fil sur les blocs, $a_1$ et $a_2$ les accélérations respectives des blocs $\ptB_1$ et $\ptB_2$, et $g$ le champ de pesanteur. Les frottements sont négligeables.
% --------------------------------------------------------- %
%                                                           %
% -------------------- Partie à droite -------------------- %
%                                                           %
...
\begin{corrige}
	Le principe fondamental appliqué au bloc $\text{B}_1$ donne $2m\vv{g}+\vv{R}+\vv{T_1}=2m \vv{a_1}$. Projetons cette relation suivant le sens du mouvement : 
	\[
...
\begin{corrige}
	Le principe fondamental appliqué au bloc $\ptB_2$ donne $m\vv{g}+\vv{T_2}=m \vv{a_2}$. Projetons cette relation suivant le sens du mouvement : 
	\[
...
\begin{enonce}
Le fil étant inextensible et sans masse on a $a_1=a_2$ et $T_1=T_2$. \\En déduire l'accélération en fonction uniquement de $g$
\end{enonce}
...
\begin{corrige}
	...
	\qquad\text{et}\qquad
	a_2=g-\frac{T_2}{m}\]
	Multiplions la première relation par $2m$, et la deuxième par $m$, puis additionnons les. On trouve $2ma_1+ma_2=T_1+mg-T_2$. Comme $a_1=a_2$ et $T_1=T_2$, on obtient $3ma_1=mg$ soit $a_1=a_2=g/3$.
\end{corrige}
