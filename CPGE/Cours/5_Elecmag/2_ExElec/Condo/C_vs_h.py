...
#axe des abscisses
nmbx = 13            #nombre de traits
xmin = 0            #valeur minimale de x
xmax = 12           #valeur maximale de x
...
""" ALU """
data = n.loadtxt('Grosse_cuve.txt')
...
(a0, a_err, b0, b_err, chi2) = fit_affine(h,     #abscisse
                                          C,     #ordonnee
...
#modelisation
plt.plot(np.linspace(0,h[-1],2),
         # T0mTa*n.exp(-x/delta) + Ta,
         b0 + a0*np.linspace(0,h[-1],2),
...
#save the figure
figsave('C_vs_h_grosse_cuve')   #changer le nom de la figure
plt.show()
