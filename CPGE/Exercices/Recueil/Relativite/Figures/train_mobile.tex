...
\begin{tikzpicture}[scale=2.0]
  \message{Worldlines^^J}
    ...
    % queue du train
    \draw[vector,mygreen] (O) -- (\psia:1.2*\xmax)
    node[mydarkgreen,above] {\contour{white}{$\rm Q$}};

    % milieu du train
    \draw[vector,mygreen] (\d,0) -- ++ (\psia:1.2*\xmax)
    node[mydarkgreen,above] {\contour{white}{$\rm M$}};

  %  tete du train
  \draw[vector,mygreen] (2*\d,0) -- ++ (\psia:1.2*\xmax)
    node[mydarkgreen,above] {\contour{white}{$\rm T$}};

    % photon
     \draw[photon,shorten >=1] (\d,0) -- ++ (45:0.43*\xmax) coordinate (TR);

      \draw[photon,shorten >=1] (\d,0) -- ++ (90+45:0.26*\xmax) coordinate (QR);

     \node at (QR) {\small$\bullet$};
     \node at (TR) {\small$\bullet$};

     % droite qui passe par QR et TR
     \draw[shorten >=-50, shorten <=-20] (QR) -- (TR) node[right=20] {$t_R'$};
    ...
    % \draw[dashed] (E) --  (\xe,0) node [below] {\contour{white}{$4$}};
    % \draw[dashed] (E) -- (0,{\ye}) node [left] {\contour{white}{$2$}};
