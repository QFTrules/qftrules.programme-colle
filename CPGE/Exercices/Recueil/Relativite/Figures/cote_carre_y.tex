...
\def\axes{
  ...
  % AXES
  \draw[<-,thick] node[above=15] {$ct$} (0,-\xmin) -- (T) ;
  \draw[<-,thick] node[right=15] {$x$} (-\xmin,0) -- (X) ;
...
\def\ang{30} % angle between x and x' axes
\begin{tikzpicture}[scale=-2.5]
  ...
  % \begin{pgfonlayer}{back} % draw on back
  %   \fill[mylightblue2] (270-\ang:0) --++ (90-\ang:\Lmax) -- ++ (0:\Lz) --++ (270-\ang:\Lmax) -- cycle;
  % \end{pgfonlayer}
  \draw[<-,thick,mygreen] (90-\ang:-\xmin) --++ (90-\ang:\Lmax+\xmin) node[below] {\contour{white}{$\rm A = \rm C$}};
  % \draw[thick,mygreen] (0,0) --++ (270-\ang:\Lmax);
  % \draw[<-,thick,myblue] ($(\Lz,0) + (90-\ang:-\xmin)$) --++ (90-\ang:\Lmax+\xmin) node[below] {\contour{white}{$\rm C$}};
  % \draw[thick,myblue] (\Lz,0) --++ (270-\ang:\Lmax);
  
  % ROD
  \pgfmathsetmacro\el{\Lz}
  \draw[<->,mybrown] (0.62,0) -- (0,0)
    node[pos=0.5,above] {\contour{white}{$\ell_y$}};
  \coordinate (E1) at (0.62,\el) ;
  % node[pos=0.5,below] {\contour{white}{$a$}};
  % \draw[rod] (\ang:{\xA/cos(\ang)}) --++ (L')
    % node[pos=0.485,above=1] {\contour{mylightblue2}{$L'$}};
  
    %  LIGHT CONE
    % \draw[photon] (45:\el*1.41) -- (0,0) ;
  ...
  \fill[mygreen] (0,0) circle(0.03) node[below right=1] {\contour{white}{$E_2$}};
  ...
  \draw[myblue,dashed] (E1) -- (0,\el) node[right] {$ct_1$};
  \fill[myblue] (E1) circle(0.03) node[left] {\contour{white}{$E_1$}};
  \coordinate (E3) at ({\el*tan(\ang)},\el);
  % \fill[mygreen] (E3) circle(0.03) node[above] {\contour{white}{$E_3$}};
  \draw[myblue,dashed] (E1) -- (0.62,0) node[above] {$x_1$};
  % \draw[<->] (0,\el) -- ++ ({\el*tan(\ang)},0) node[midway,below] {\contour{white}{$vt_1$}};
