...
\def\axes{
  ...
  \draw[->,thick,mydarkred] (90-\ang:-\xmin) -- (T')
    ...
    \begin{scope}[xshift=2.35*\d cm]
      \draw[thick,mydarkred] (90-\ang:-\xmin) -- (90-\ang:\ymaxp+0.2)
      node[right=5,above=-1] {$\mathcal{M}_1$};
    \end{scope}
...
\begin{tikzpicture}[scale=2]
  ...
  % \draw[mygreen,dashed,rounded corners=0.1]
    ...
    % \draw[photon]
    %  (C) -- (D);
    \draw[photon] (0,0) -- ++ (45:1.95) coordinate (E);
    \draw[photon] (E) -- ++ (135:0.7) coordinate (F);
    ...
    \draw[myblue,dashed,thick,rounded corners=0.1]
    (E)   -- ++ (\ang:-1) node[left] {\contour{white}{$\ell$}}; 
\draw[myblue,dashed,thick,rounded corners=0.1]
(E)   -- ++(90-\ang:-1) node[below right=-3] {\contour{white}{$\ell$}}; 
\draw[dashed] (E) -- ++(0,-1.4) node[below] {\contour{white}{$\gamma(1+\beta)\ell$}};
\draw[dashed] (E) -- ++(-1.4,0) node[left] {\contour{white}{$\gamma(1+\beta)\ell$}};
\draw[dashed,mydarkgreen,thick] (F) -- ++(-0.89,0) node[left,mygreen] {\contour{white}{$2\gamma\ell$}};

% EVENTS
\fill[mygreen] (F) circle(0.03) node[above=5,left=-2] {\contour{white}{$R_1$}};
\fill[myblue] (A) circle(0.03) node[below=8, right=0] {\contour{white}{$E_0$}};
\fill[myblue] (E) circle(0.03) node[below=1,right=0] {\contour{white}{$A_1$}};
\end{tikzpicture}
