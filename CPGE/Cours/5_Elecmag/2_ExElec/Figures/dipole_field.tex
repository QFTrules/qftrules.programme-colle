...
\colorlet{Bcol}{orange!90!black}
\colorlet{BcolFL}{orange!80!black}
...
\contourlength{1.5pt}
\tikzset{
   EFielLineArrow/.style args={#1,#2}{BcolFL,decoration={markings,
...
\newcommand{\EFieldLines}{ % ELECTRIC FIELD LINES
  ...
  % FIELD LINES
  \draw[BcolFL,name path=Elines] plot[id=plot, raw gnuplot, smooth]
    function{
...
\begin{tikzpicture}
  ...
  \def\Q{1.0}
  \def\R{1.9}
  \def\range{0.003,0.012,0.03,0.045,0.07,0.1}
  %\def\range{0.08,0.16,0.24,0.30,0.36}
  % \def\range{0.025,0.15,0.3,0.6,0.8,1.0,1.4,1.8,1.98}

  % \begin{scope}[rotate=90,shift={(0,0)}]
    \EFieldLines
    % \draw[thick,EFielLineArrow={0.65,180}] (0,0) -- (0,-\ymax);
    % \draw[thick,EFielLineArrow={0.65,0}] (0,0) -- (0,\ymax);
  % \end{scope}
  % draw electric moment p
  \draw[->,Ecol,ultra thick,blue!90!black] (0,-0.8) -- (0,0.8) node[right,rotate=90] {\contour{white}{\large $\Vec{p}$}};
