...
# draw vertical line at x=20  between y=20 and y=80
plt.axvline(x=20, color='r', linestyle='-', ymin=(5 - ymin)/(ymax - ymin), ymax=(80 - ymin)/(ymax - ymin))

plt.plot(x1,
            # T0mTa*n.exp(-x/delta) + Ta,
            yconsigne1*np.ones(len(x1)),
            'r-',
            label=r'$e(t)$'
            )
plt.plot(x2,
            # T0mTa*n.exp(-x/delta) + Ta,
            yconsigne2*np.ones(len(x2)),
            'r-'
            )
# """ CUIVRE """
...
#save the figure
figsave('tau_mesure')   #changer le nom de la figure
plt.show()
