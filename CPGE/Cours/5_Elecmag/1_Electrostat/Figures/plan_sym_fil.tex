...
\begin{tikzpicture}
  ...
  % \draw[charge+] (-\a,0) circle (9pt) node[black,scale=1.0] {$+q$};
  % \draw[charge+] (+\a,0) circle (9pt) node[black,scale=1.0] {$+q$};
  \fill[red,draw opacity=0] (-\R,-\largeur) rectangle ++(\R,2*\largeur);
  ...
  \def\larg{0.3}
  \fill[gray,opacity=0.6,draw opacity=1,draw=black] (-\larg,-\R) -- ++ (0,2*\R) -- ++ (2*\larg,0.5*\larg) -- ++ (0,-2*\R) -- cycle node[midway,below,black,opacity=1] {$\Pi$};
  \fill[red,draw opacity=0] (0,-\largeur) rectangle ++(\R,2*\largeur);
  \foreach \x in {0.2,0.6,...,\R}{
    % draw a cross symbol
    \draw[] (\x,-0.1) -- ++(0,0.2);
    \draw[] (\x-0.1,0) -- ++(0.2,0);
...
% plan d'antisymétrie
  ...
  % \fill[gray,opacity=0.1,draw opacity=1,draw=black] (-\larg,-\R) -- ++ (0,2*\R) -- ++ (2*\larg,0.5*\larg) -- ++ (0,-2*\R) -- cycle node[midway,below,black,opacity=1] {$\Pi$};
