...
\begin{tikzpicture}[scale=2.0]
  \message{Worldlines^^J}
    ...
    \coordinate (E) at (-\xe,{\xe*2});  
    \coordinate (F) at ({2*\xe},{-\xe});
    % \node at (E) [mydarkpurple] {$\bullet$};
    % \node at (E) [above=3,mydarkpurple] {\contour{white}{$E$}};
    \draw[dashed] (E) --  (-\xe,0) node [below] {\contour{white}{$-1$}};
    \draw[dashed] (E) -- (0,{\xe*2}) node [right] {\contour{white}{$2$}};
    \draw[dashed] (F) --  ({2*\xe},0) node [above] {\contour{white}{$2$}};
    \draw[dashed] (F) -- (0,{-\xe}) node [left] {\contour{white}{$-1$}};

    % angle psia de 27°
    \draw[->,>=latex,mygreen,thick] ({3.5*\xe},0) arc(0:-27:{3.5*\xe}) node[right,midway] {$\SI{27}{^\circ}$};
