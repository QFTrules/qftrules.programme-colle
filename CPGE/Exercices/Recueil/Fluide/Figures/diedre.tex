...
\begin{tikzpicture}
  ...
  % draw two metal planes making an angle alpha
  \fill[metal] (-3,-1.25)  rectangle ++(5,0.2);
  \begin{scope}[rotate around={35:(-3,-1)}]
    \fill[metal] (-3,-1.25)  rectangle ++(5,0.2);
  \end{scope}

  % flow between the planes
  \draw[myblue,thick,postaction={decorate},decoration={markings,
        mark=at position {0.2} with {\arrow{latex}},mark=at position 0.9 with {\arrow{latex}}}] (2,-0.75) .. controls ++(-5,0) and ++(-2,-1.5) .. ++ (-1,2);
  \draw[myblue,thick,postaction={decorate},decoration={markings,
        mark=at position {0.2} with {\arrow{latex}},mark=at position 0.9 with {\arrow{latex}}}] (2,-0.5) .. controls ++(-2,0) and ++(-2,-1) .. ++ (-1,1.25);
  % \draw[metal]
  %   (0,0) circle(\R);
  % \foreach \s in {-1,1}{
